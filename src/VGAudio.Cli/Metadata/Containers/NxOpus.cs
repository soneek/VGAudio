...
{
    internal class NxOpus : MetadataReader
    {
        public override object ReadMetadata(Stream stream)
        {
            return new NxOpusReader().ReadMetadata(stream);
        }

        public override Common ToCommon(object structure)
        {
            var nxOpus = structure as NxOpusStructure;
            if (nxOpus == null) throw new InvalidDataException("Could not parse file metadata.");
            //NxOpusStructure info = nxOpus.NxOpusStructure;

            return new Common
            {
                SampleCount = nxOpus.SampleCount,
                SampleRate = nxOpus.SampleRate,
                ChannelCount = nxOpus.ChannelCount,
                Format = AudioFormat.Opus,
                Looping = nxOpus.Looping,
                LoopStart = nxOpus.LoopStart,
                LoopEnd = nxOpus.SampleCount
            };
        }
